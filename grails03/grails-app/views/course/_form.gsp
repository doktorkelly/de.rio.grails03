...
<div class="fieldcontain ${hasErrors(bean: courseInstance, field: 'group', 'error')} ">
	<label for="group">
		<g:message code="course.group.label" default="Group" />
		
	</label>
	<g:textField name="group" value="${courseInstance?.group}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: courseInstance, field: 'units', 'error')} ">
	<label for="units">
		<g:message code="course.units.label" default="Units" />
		
	</label>
	<g:field name="units" value="${fieldValue(bean: courseInstance, field: 'units')}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: courseInstance, field: 'room', 'error')} ">
	...
	<g:select id="room" name="room.id" from="${de.rio.Room.list()}" optionKey="id" value="${courseInstance?.room?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>

<div class="fieldcontain ${hasErrors(bean: courseInstance, field: 'quartal', 'error')} ">
	<label for="quartal">
		<g:message code="course.quartal.label" default="Quartal" />
		
	</label>
	<g:textField name="quartal" value="${courseInstance?.quartal}"/>
</div>
...
<g:each in="${courseInstance?.registrations?}" var="r">
    <li><g:link controller="course" action="editReg" id="${r.id}">${r?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="course" action="createReg" params="['course.id': courseInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'registration.label', default: 'Registration')])}</g:link>
</li>
</ul>
